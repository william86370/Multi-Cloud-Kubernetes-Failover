...
def create_daemon_set_object():
    container = client.V1Container(
        ...
        image="ghcr.io/william86370/cloudwatch-daemon:latest",
        image_pull_policy="IfNotPresent",
        ports=[client.V1ContainerPort(container_port=8080, name="http")],
    )
    # Template
    template = client.V1PodTemplateSpec(
        metadata=client.V1ObjectMeta(labels={"app": "cloudwatch"}),
        spec=client.V1PodSpec(containers=[container]))
    # Spec
    spec = client.V1DaemonSetSpec(
        selector=client.V1LabelSelector(
            match_labels={"app": "cloudwatch"}
        ),
        template=template)
    # DaemonSet
    daemonset = client.V1DaemonSet(
        api_version="apps/v1",
        kind="DaemonSet",
        metadata=client.V1ObjectMeta(name="daemonset-cloudwatch"),
        spec=spec)
...
def update_daemon_set(apps_v1_api, daemonset):
    ...
    daemonset.spec.template.spec.containers[0].image = "ghcr.io/william86370/cloudwatch-daemon:latest"
...
def main():
    ...
    controller_revisions = list_controller_revision(
        apps_v1_api, "default", "daemonset-cloudwatch")
